    ...
    >
      <div className="relative aspect-[4/3] overflow-hidden bg-slate-100">
        {product.image_url ? (
          ...
          <div className="flex h-full flex-col items-center justify-center gap-2 bg-[radial-gradient(circle_at_50%_35%,#ffffff_0,#f1f5f3_58%,#e2e8e5_100%)] text-slate-400">
            <span className="flex h-11 w-11 items-center justify-center rounded-full border border-slate-200 bg-white text-xl font-light text-emerald-700 shadow-sm" aria-hidden="true">+</span>
            <span className="text-[10px] font-bold uppercase tracking-[0.16em]">No image</span>
          </div>
        )}
      ...
      <div className="flex flex-1 flex-col p-4 sm:p-5">
        <div className="flex items-start justify-between gap-3">
          <h2 className="line-clamp-2 text-[15px] font-bold leading-5 text-slate-900">{product.name}</h2>
        ...
        <div className="mt-auto flex items-center gap-2 border-t border-slate-100 pt-4">
          <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-emerald-50 text-[11px] font-black text-emerald-700">{sellerName.charAt(0).toUpperCase()}</span>
          <p className="min-w-0 truncate text-xs font-semibold text-slate-700">
            {sellerName}
            {seller.department ? <span className="block truncate text-[11px] font-normal text-slate-400">{seller.department}</span> : null}
          </p>
        </div>
      </div>
    </Link>
